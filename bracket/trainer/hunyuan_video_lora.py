"""HunyuanVideo LoRA — wraps musubi-tuner hv_train_network.

HunyuanVideo is the 13B video DiT from Tencent. Dataset TOML must be in the
musubi video shape (frame_buckets + video_directory). Sampling produces .mp4
clips; bracket extracts representative frames before VLM judging.
...
@dataclass
class HunyuanVideoLoRAConfig(TrainerConfig):
    ...
    discrete_flow_shift: float = 7.0  # video models use much higher shifts
...
class HunyuanVideoLoRATrainer(Trainer):
    name = "hunyuan-video-lora-musubi"
    ...
    def __init__(
        ...
        *,
        ...
        self.train_script = self.musubi_dir / "src" / "musubi_tuner" / "hv_train_network.py"
        if not self.train_script.exists():
            self.train_script = self.musubi_dir / "hv_train_network.py"
        if not self.train_script.exists():
            raise FileNotFoundError(
                f"hv_train_network.py not found under {self.musubi_dir}"
            )
    ...
    def declare_search_space(self) -> SearchSpace:
        if lora_grad_ckpt_varies(self.tier, "hunyuan-video"):
            ckpt_knob = CategoricalKnob(choices=(True, False))
        else:
            ckpt_knob = FixedKnob(value=True)
        return SearchSpace(
            name=f"hunyuan-video-lora-v0.1-{self.tier}",
            knobs={
                ...
                "fp8_llm": FixedKnob(value=True),
                "blocks_to_swap": FixedKnob(value=BLOCKS_TO_SWAP_BY_TIER[self.tier]),
            },
        )

    def baseline_config(self) -> HunyuanVideoLoRAConfig:
        return HunyuanVideoLoRAConfig(
            train_batch_size=VIDEO_LORA_DEFAULT_BATCH_BY_TIER[self.tier],
            gradient_checkpointing=lora_grad_ckpt_baseline(self.tier, "hunyuan-video"),
    ...
    def curated_configs(self) -> list[TrainerConfig]:
        ...
        return [
            # 1) musubi docs starter — flow_shift=7 is the documented default.
            ...
            # 2) Lower flow_shift for short-clip datasets.
            HunyuanVideoLoRAConfig(
                learning_rate=5e-5, optimizer_type="AdamW8bit",
                lr_scheduler="cosine", lr_warmup_steps=100,
                network_dim=32, network_alpha=16.0,
    ...
    def config_from_dict(self, knobs: Mapping[str, Any]) -> HunyuanVideoLoRAConfig:
        return HunyuanVideoLoRAConfig(
    ...
    def prepare_run(
        ...
        *,
        ...
        if not isinstance(config, HunyuanVideoLoRAConfig):
            raise TypeError(f"expected HunyuanVideoLoRAConfig, got {type(config).__name__}")
...
def _hv_pre_cache_commands(
    *,
    ...
    """HunyuanVideo / FramePack pre-cache: dual TE (LLaMA3 + CLIP-L)."""
