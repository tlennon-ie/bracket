"""Wan 2.1 / 2.2 full fine-tune — wraps musubi-tuner wan_train.

Same shape as HunyuanVideo full FT. Uses the single Wan T5 (UMT5-XXL) and the
`--task` knob to select t2v / i2v / t2i mode.
"""
...
from bracket.trainer.wan_lora import _wan_pre_cache_commands
...
@dataclass
class WanFullConfig(TrainerConfig):
...
class WanFullTrainer(Trainer):
    name = "wan-full-musubi"
    ...
    def __init__(
        ...
        *,
        ...
        self.task = task
        self.train_script = self.musubi_dir / "src" / "musubi_tuner" / "wan_train.py"
        if not self.train_script.exists():
            self.train_script = self.musubi_dir / "wan_train.py"
        if not self.train_script.exists():
            raise FileNotFoundError(f"wan_train.py not found under {self.musubi_dir}")
    ...
    def declare_search_space(self) -> SearchSpace:
        return SearchSpace(
            name=f"wan{self.wan_version.replace('.', '')}-full-v0.1-{self.tier}",
            knobs={
    ...
    def baseline_config(self) -> WanFullConfig:
        return WanFullConfig(blocks_to_swap=_FULL_SWAP_BY_TIER[self.tier])
    ...
    def curated_configs(self) -> list[TrainerConfig]:
        ...
        return [
            WanFullConfig(
                ...
                lr_warmup_steps=50, discrete_flow_shift=5.0,
                blocks_to_swap=swap,
            ),
            WanFullConfig(
                learning_rate=1e-5, lr_scheduler="cosine",
                lr_warmup_steps=200, discrete_flow_shift=5.0,
                blocks_to_swap=swap,
            ),
        ]

    def config_from_dict(self, knobs: Mapping[str, Any]) -> WanFullConfig:
        return WanFullConfig(
    ...
    def prepare_run(
        ...
        *,
        ...
        if not isinstance(config, WanFullConfig):
            raise TypeError(f"expected WanFullConfig, got {type(config).__name__}")
