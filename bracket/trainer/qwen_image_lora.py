"""Qwen-Image LoRA trainer adapter — wraps musubi-tuner qwen_image_train_network.

Qwen-Image is the 20B MMDiT image model from Alibaba. The musubi adapter follows
the same conventions as Z-Image / Flux-2:
  ...
  - fp8_base / fp8_scaled both available; on a 32 GB card you'll typically
    want at least one of them on, plus blocks_to_swap if VRAM is tight.

Module names used by musubi-tuner upstream:
  train:      musubi_tuner.qwen_image_train_network
  cache lat:  musubi_tuner.qwen_image_cache_latents
  cache TE:   musubi_tuner.qwen_image_cache_text_encoder_outputs

The text encoder for Qwen-Image is Qwen2.5-VL-7B (NOT plain Qwen3) — keep the
two paths distinct in the registry.
"""
...
@dataclass
class QwenImageLoRAConfig(TrainerConfig):
    ...
    fp8_vl: bool = False                 # cuts VL TE precision, saves ~1 GB
    gradient_checkpointing: bool = True
    blocks_to_swap: int = 0
    dataloader_workers: int = 2


class QwenImageLoRATrainer(Trainer):
    name = "qwen-image-lora-musubi"
    ...
    def declare_search_space(self) -> SearchSpace:
        if lora_grad_ckpt_varies(self.tier, "qwen-image"):
            ckpt_knob = CategoricalKnob(choices=(True, False))
        else:
            ckpt_knob = FixedKnob(value=True)
        return SearchSpace(
            name=f"qwen-image-lora-v0.1-{self.tier}",
            knobs={
    ...
    def baseline_config(self) -> QwenImageLoRAConfig:
        return QwenImageLoRAConfig(
            learning_rate=1e-4,
            train_batch_size=SDXL_LORA_DEFAULT_BATCH_BY_TIER[self.tier],
            gradient_checkpointing=lora_grad_ckpt_baseline(self.tier, "qwen-image"),
    ...
    def curated_configs(self) -> list[TrainerConfig]:
        ...
        ckpt = lora_grad_ckpt_baseline(self.tier, "qwen-image")
        workers = MUSUBI_DATALOADER_WORKERS_BY_TIER[self.tier]
        swap = BLOCKS_TO_SWAP_BY_TIER[self.tier]
        return [
            # 1) Conservative starter — Qwen-Image 20B trains slowly, 5e-5 is
            #    the community-reported safe LR.
            QwenImageLoRAConfig(
            ...
            # 2) Prodigy auto-LR — usually wins on Qwen-Image.
            QwenImageLoRAConfig(
            ...
            # 3) Higher flow_shift — Qwen-Image benefits from elevated shift on
            #    portrait-style datasets.
            QwenImageLoRAConfig(
    ...
    def config_from_dict(self, knobs: Mapping[str, Any]) -> QwenImageLoRAConfig:
        return QwenImageLoRAConfig(
    ...
    def prepare_run(
        ...
        *,
        ...
        if not isinstance(config, QwenImageLoRAConfig):
            raise TypeError(f"expected QwenImageLoRAConfig, got {type(config).__name__}")
