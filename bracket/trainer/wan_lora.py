"""Wan 2.1 / 2.2 LoRA — wraps musubi-tuner wan_train_network.

Wan is Alibaba's video DiT family. Both 2.1 and 2.2 14B variants share the
same trainer surface in musubi-tuner — the difference is the DiT weights file
the user supplies. The 1.3B variant works the same way.

`wan_version` selects flow-matching defaults:
  - "2.1"  → discrete_flow_shift baseline ~5.0
  - "2.2"  → discrete_flow_shift baseline ~5.0 (HighNoise/LowNoise dual files
             are loaded via the user's --dit path; bracket doesn't intervene)

Module names (musubi-tuner upstream):
  train:      musubi_tuner.wan_train_network
  cache lat:  musubi_tuner.wan_cache_latents
  cache TE:   musubi_tuner.wan_cache_text_encoder_outputs

Wan uses a single text encoder: UMT5-XXL.

`task` controls Wan's training task (`t2v-14B`, `i2v-14B`, `t2i-14B`, etc).
Default `t2v-14B` covers the most common LoRA use case.
"""
...
@dataclass
class WanLoRAConfig(TrainerConfig):
...
class WanLoRATrainer(Trainer):
    name = "wan-lora-musubi"
    ...
    def __init__(
        ...
        *,
        ...
        if wan_version not in ("2.1", "2.2"):
            raise ValueError(f"wan_version must be '2.1' or '2.2', got {wan_version!r}")
        self.wan_version = wan_version
        self.task = task
        model_class = "wan21" if wan_version == "2.1" else "wan22"
        self.model_class = model_class
        self.train_script = self.musubi_dir / "src" / "musubi_tuner" / "wan_train_network.py"
        if not self.train_script.exists():
            self.train_script = self.musubi_dir / "wan_train_network.py"
        if not self.train_script.exists():
            raise FileNotFoundError(
                f"wan_train_network.py not found under {self.musubi_dir}"
            )
    ...
    def declare_search_space(self) -> SearchSpace:
        ...
        return SearchSpace(
            name=f"wan{self.wan_version.replace('.', '')}-lora-v0.1-{self.tier}",
            knobs={
                ...
                "discrete_flow_shift": FloatKnob(low=3.0, high=8.0),
                ...
                "fp8_t5": FixedKnob(value=self.tier in ("low", "tiny", "med")),
                "blocks_to_swap": FixedKnob(value=BLOCKS_TO_SWAP_BY_TIER[self.tier]),
            },
        )

    def baseline_config(self) -> WanLoRAConfig:
        return WanLoRAConfig(
            ...
            fp8_t5=self.tier in ("low", "tiny", "med"),
        )
    ...
    def config_from_dict(self, knobs: Mapping[str, Any]) -> WanLoRAConfig:
        return WanLoRAConfig(
    ...
    def session_setup_commands(self, *, dataset_toml: Path, run_dir: Path) -> list[LaunchSpec]:
        return _wan_pre_cache_commands(
    ...
    def prepare_run(
        ...
        *,
        ...
        if not isinstance(config, WanLoRAConfig):
            raise TypeError(f"expected WanLoRAConfig, got {type(config).__name__}")
...
def _wan_pre_cache_commands(
    *,
    ...
    """Wan pre-cache: single TE (UMT5-XXL via --t5)."""
    ...
    return [
        LaunchSpec(
            cmd=[
                str(venv_python), "-m", "musubi_tuner.wan_cache_latents",
                "--task", task,
        ...
        LaunchSpec(
            cmd=[
                str(venv_python), "-m", "musubi_tuner.wan_cache_text_encoder_outputs",
                "--dataset_config", str(flat_toml),
                "--t5", text_encoder_path,
            ],
            cwd=musubi_dir, env=env,
            output_dir=run_dir, logging_dir=logging_dir,
            tfevents_glob="",
