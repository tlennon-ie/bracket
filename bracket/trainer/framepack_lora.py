"""FramePack LoRA — wraps musubi-tuner fpack_train_network.

FramePack is a HunyuanVideo-derived model that conditions on a single anchor
frame. Same dual-TE shape as HunyuanVideo (LLaMA3 + CLIP-L), same VAE family,
but a separate trainer module.

Module names (musubi-tuner upstream):
  train:      musubi_tuner.fpack_train_network
  cache lat:  musubi_tuner.fpack_cache_latents
  cache TE:   musubi_tuner.fpack_cache_text_encoder_outputs
"""
...
@dataclass
class FramePackLoRAConfig(TrainerConfig):
...
class FramePackLoRATrainer(Trainer):
    name = "framepack-lora-musubi"
    ...
    def __init__(
        ...
        *,
        ...
        self.train_script = self.musubi_dir / "src" / "musubi_tuner" / "fpack_train_network.py"
        if not self.train_script.exists():
            self.train_script = self.musubi_dir / "fpack_train_network.py"
        if not self.train_script.exists():
            raise FileNotFoundError(
                f"fpack_train_network.py not found under {self.musubi_dir}"
            )
    ...
    def declare_search_space(self) -> SearchSpace:
        if lora_grad_ckpt_varies(self.tier, "framepack"):
            ckpt_knob = CategoricalKnob(choices=(True, False))
        else:
            ckpt_knob = FixedKnob(value=True)
        return SearchSpace(
            name=f"framepack-lora-v0.1-{self.tier}",
            knobs={
    ...
    def baseline_config(self) -> FramePackLoRAConfig:
        return FramePackLoRAConfig(
            train_batch_size=VIDEO_LORA_DEFAULT_BATCH_BY_TIER[self.tier],
            gradient_checkpointing=lora_grad_ckpt_baseline(self.tier, "framepack"),
    ...
    def config_from_dict(self, knobs: Mapping[str, Any]) -> FramePackLoRAConfig:
        return FramePackLoRAConfig(
    ...
    def session_setup_commands(self, *, dataset_toml: Path, run_dir: Path) -> list[LaunchSpec]:
        return _hv_pre_cache_commands(
            ...
            cache_latents_module="musubi_tuner.fpack_cache_latents",
    ...
    def prepare_run(
        ...
        *,
        ...
        if not isinstance(config, FramePackLoRAConfig):
            raise TypeError(f"expected FramePackLoRAConfig, got {type(config).__name__}")
